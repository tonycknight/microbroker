...
module WebApiValidationTests =
    ...
    let nonEmptyAlphanumerics =
        strings |> Arb.filter (Seq.forall Char.IsLetterOrDigit) |> Arb.filter ((<>) "")

    let validName (value: string) =
        let validNameCharacters =
            [ 'a' .. 'z' ] @ [ '0' .. '9' ] @ [ '-'; '_' ] |> Array.ofSeq

        value.Length > 0
        && Strings.lower value
           |> Seq.forall (fun c -> Array.contains c validNameCharacters)

    [<Property>]
    let ``isValidQueueName empty strings return false`` () =
        Prop.forAll strings ((String.IsNullOrEmpty >> not) ||>> (WebApiValidation.isValidQueueName >> not))
    ...
    let ``isValidQueueName whitespace strings return false`` () =
        Prop.forAll
            ...
            ((String.IsNullOrWhiteSpace >> not)
             ||>> (WebApiValidation.isValidQueueName >> not))
    ...
    let ``isValidQueueName case insensitive alphanumeric strings return true`` () =
        Prop.forAll
            nonEmptyAlphanumerics
            ((Strings.lower >> WebApiValidation.isValidQueueName)
             =>> (Strings.lower >> WebApiValidation.isValidQueueName))
    ...
    let ``validateQueueName finds invalid names`` () =
        ...
        Prop.forAll names (fun n ->
            match WebApiValidation.validateQueueName n with
            | Choice2Of2 _ -> false
            | Choice1Of2 _ -> true)
